...
contract PearlVault is RewardsDistributionRecipient, ReentrancyGuard, Pausable {
        ...
        returns (uint256)
    {
        return
            boostPointOf(termIndex, tokenId)
                .mul(
                    rewardPerToken().sub(
                        noteRewardPerTokenPaid[termIndex][tokenId]
                    )
                )
                .div(1e18)
    ...
    function _updateReward(uint256 termIndex, uint256 tokenId) private {
        ...
        if (termIndex != type(uint256).max) {
            ...
            noteRewardPerTokenPaid[termIndex][tokenId] = rewardPerTokenStored;
        }
    }
